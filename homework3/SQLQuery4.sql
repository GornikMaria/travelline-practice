...
CREATE TABLE Car (
    IdCar INT IDENTITY(1,1) NOT NULL CONSTRAINT PK_Сar PRIMARY KEY,
    Name NVARCHAR(100) NOT NULL,
...
-- Client
INSERT INTO Client (Birthday, Name, Address) VALUES ('2020-12-12', 'Александр', 'Успенская, д.13');
INSERT INTO Client (Birthday, Name, Address) VALUES ('1940-02-02', 'Владимир', 'Крайовская, д.42');
INSERT INTO Client (Birthday, Name, Address) VALUES ('2001-08-04', 'Петр', 'Уличная, д.2');
...
SELECT * FROM Client WHERE Name = 'Александр';
SELECT * FROM Client WHERE Address != 'Молодежная, д. 5';
...
SELECT * FROM Client WHERE Name LIKE '%ет%';
SELECT * FROM Car WHERE Price > 30000 AND Rating = 3;
SELECT * FROM Client WHERE Name = 'Петр' OR Name = 'Александр';

-- 3.3 UPDATE
UPDATE Client SET Address = 'Молодежная, д. 5' WHERE IdClient = 1;
